         ...
         if self.sonar_steer:
            if not self.cliff:
               ...
               if cur_left_range > 0.1 and cur_right_range > 0.1:
                  rospy.loginfo("left_sonar:%s right_sonar:%s",cur_left_range,cur_right_range)
               
                  if cur_left_range < 0.3 and cur_left_range > 0.1:
                  ...
                  elif cur_right_range < 0.3 and cur_left_range > 0.1: 
                     ...
                     if cur_right_range < cur_left_range and abs(cur_right_range-cur_left_range) >0.1:
                        if front_curpos > -1.05:
                           front_newpos = front_curpos - 0.17
                           rear_newpos = front_newpos *-1
                           rospy.loginfo("LEFT TURN:left_sonar:%s right_sonar:%s",cur_left_range,cur_right_range)
                        else:
                           front_newpos = -1.05
                           rear_newpos = 1.05

                     if cur_left_range < cur_right_range and abs(cur_right_range-cur_left_range) >0.1:
                        if front_curpos < 1.05:
                           front_newpos = front_curpos + 0.17
                           rear_newpos = front_newpos *-1
                           rospy.loginfo("RIGHT TURN:left_sonar:%s right_sonar:%s",cur_left_range,cur_right_range)
                        else:
                           front_newpos = 1.05
                           rear_newpos = -1.05 
                  #for s in self.sonar:
                  #   if s] < self.sonar_warn:
                  ...
                  #This is a pretty greenfield opportunity to actually start using smart(er) control systems
